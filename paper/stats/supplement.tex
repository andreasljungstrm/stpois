...
global claim; I state this boundary explicitly since it is where the
...
The following theorem resolves the practical question for the present
setting:
...
MLE in the relevant regime.  I give results in three regimes,
...
\citet{gourieroux1984}; I include it because the triangular-array
...
The regime that motivates the proportional-dimension question is more
extreme:
...
theory, which I now state.  Conditional on covariates and cell effects,
...
exponential dispersion family; I record the two most used instances.
...
computational barrier to evaluating them inside the tilted pass, and I
...
disconnected ($\rho = 0.99$).  At Poisson optimum weights I compute the
...
$\rho$ I compute the exact level-space spectral rate of
...
probability.  I compare the MLE (tilted Newton, non-convergence or
...
I compare the profile MLE (inner concentration of the cell effects by
...
the asymptotic message concrete: the proportional regime is not
...
(every block updated at least once every $B$ iterations).  In
particular, running the inner $\balpha$-loop to tolerance before each
outer $\bbeta$-step (as Algorithm~\ref{alg:absorb} does) is an instance
of an essentially-cyclic ordering: each $\balpha$-block is updated at
least once, and the $\bbeta$-block exactly once, per outer cycle, so the
vanishing-per-block-increment argument applies verbatim with $B$ equal
to the (finite, since the inner loop converges $R$-linearly by
Theorem~\ref{thm:rate}(i)) inner-loop length.
...
Under Assumption~\ref{ass:classical}, the divergence and normalization
conditions of \citet[Sec.~3]{fahrmeir1985} hold for the canonical-link
exponential family with bounded covariates---namely
$\lambda_{\min}(\bF_N) \to \infty$ and
$\bF_N^{-1/2}\bF_N(\btheta)\bF_N^{-1/2} \to \bI$ uniformly on shrinking
$\bF_N$-neighborhoods of $\btheta_0$, which follow from
Assumption~\ref{ass:classical}(i)--(ii) (bounded covariates with
$b \in C^3$ give the uniform third-derivative control, and
$N^{-1}\bF_N \to \bF_\infty \succ \bzero$ gives the normalization).
