...
convergence to the maximum-likelihood estimate for canonical links---%
resolving a gap repeatedly noted in the applied literature---and derive
its local linear rate, which for two absorbed factors equals the squared
Friedrichs angle cosine between the weighted fixed-effect subspaces and,
equivalently, one minus the spectral gap of a weighted bipartite graph
Laplacian.  We then develop asymptotic theory in three regimes: fixed
...
incidental-parameter bias.  Extensions cover logistic and gamma models,
...
precision at a small fraction of the cost, that observed convergence rates
match the spectral prediction to three decimals, and that nominal $95\%$
intervals attain $94$--$96\%$ coverage even with one fixed effect per two
observations.  A register-scale case study with five million person-year
...
designs).  Theorem~\ref{thm:rate} derives the local convergence rate: the
iteration contracts linearly with factor equal to the spectral radius of
the block Gauss--Seidel operator of the information matrix at the
optimum; for two absorbed factors this radius equals
...
\citep{gaure2013}.  Simulations in Section~\ref{sec:sim-rate} match the
predicted rates to three decimal places.
...
discussed.  Simulations with one fixed effect per \emph{two} observations
show nominal $95\%$ intervals covering at $94$--$96\%$
(Section~\ref{sec:sim-highdim}).
...
cyclic or essentially-cyclic ordering of the blocks.
\end{theorem}
...
\citep{andersen1970,fernandezval2016}) is identically zero here.  This is
...
Theorems~\ref{thm:global}--\ref{thm:rate} apply verbatim (the logit link
is canonical).  Unlike the Poisson case the tilt depends on $\bdelta$
through $\omega_i$, so the within-cell moments must be re-accumulated
each iteration---which the single pass does anyway.  The proportional-regime
theory of Section~\ref{sec:proportional} does \emph{not} transfer:
profiling and conditioning differ for the logit, conditional ML requires
the permanent-type sums of \citet{andersen1970}, and profile ML carries
$O(1/m)$ incidental-parameter bias \citep{fernandezval2016}; the honest
options at $J/N \to c$ are conditional ML (computable but heavier) or
bias correction, and we view the integration of either into the
tilted-moment pass as valuable future work.
...
likelihood a cell-level computation; global convergence and sharp local
rates for the nonlinear Gauss--Seidel absorption cycle
(Theorems~\ref{thm:global}--\ref{thm:rate}), with the rate identified as
a bipartite spectral gap computable before iterating
(Corollary~\ref{cor:bipartite}); and asymptotics that cover the
...
(Theorem~\ref{thm:proportional}).

Several limitations delineate future work.  First, the
proportional-regime theory is Poisson-specific; for the logit the
conditional likelihood exists but is not the profile likelihood, and
integrating conditional-ML or analytic bias corrections
\citep{fernandezval2016} into the tilted pass is open.  Second,
Theorem~\ref{thm:rate} characterizes the local rate for $G = 2$
absorbed factors exactly; for $G \ge 3$ the block Gauss--Seidel radius
bound is available but the clean angle characterization is not, matching
the state of the linear alternating-projections literature
\citep{deutsch2001}.  Third, our separation handling is exact for the
...
(every block updated at least once every $B$ iterations). \qed
