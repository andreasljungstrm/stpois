...
explicit dummy variables, owing to the quadratic growth in memory and
computation time with the parameter count, and continuous covariates
prevent the collapsing of individual records to risk cells that
otherwise makes such models fast. This article introduces the
community-contributed command \stcmd{stpois}, which fits Poisson
event-history models directly on \stcmd{stset} data through three
estimation paths, all of which return maximum-likelihood estimates that
agree with \stcmd{poisson} to at least \(10^{-6}\): a standard path that
wraps \stcmd{poisson} with automatic exposure; a high-dimensional
...
the cell structure of the categorical covariates. The fast path computes
each Newton iteration from exponentially tilted within-cell moments in a
single pass over the microdata. Coefficients, log likelihoods, and
observed-information, robust, and cluster--robust standard errors agree
with the corresponding \stcmd{poisson} model on every path to at least
\(10^{-6}\) (typically \(\sim 10^{-8}\)); factor-variable interactions
and weights are supported throughout, the two acceleration mechanisms
can be combined, and postestimation predictions after absorption include
the recovered fixed effects. Benchmarks against \stcmd{streg},
\stcmd{poisson}, and \stcmd{ppmlhdfe} document the speed gains in three
settings: episode-split workflows, all-categorical rate models, and
...
when the fixed effects are entered as explicit dummy variables, because
both estimation time and memory grow quadratically with the parameter
count (and because Stata's dummy-variable trap limits the practical
number of indicator columns). The \stcmd{ppmlhdfe} command
\citep{correia2020} solves this problem for general Poisson models,
building on the alternating-projections approach of
\citet{guimaraes2010}; it depends on the \stcmd{ftools} and
...
the log likelihood guards the updates: if the full Newton step does not
increase the log likelihood, the step length is halved repeatedly until
an improvement is found or the step shrinks to \(2^{-16}\) of its
original length, at which point the algorithm accepts the current
iterate and continues; if the outer convergence criterion is still not
met after \stcmd{maxiter()} such steps, \stcmd{e(converged)} is set to
\(0\). Writing \(\bm{\theta} = (\bm{\gamma}', \bm{\delta}')'\) for the
stacked parameter vector, convergence is declared when
...
maximum-likelihood estimates. For very large datasets, running
\stcmd{compress} before estimation reduces the in-memory footprint of
the loaded columns; the Mata engine creates no additional dataset
variables beyond the single sort permutation.
...
\stcmd{pweight}s imply robust standard errors (see option descriptions
below), and weighted coefficients and standard errors agree with the
corresponding weighted \stcmd{poisson} model to at least \(10^{-6}\).
\stcmd{fast} and \stcmd{absorb()} may be combined
...
\stcmd{stpois} supports the \stcmd{svy:} prefix for design-based
variance estimation. \stcmd{svy: stpois} produces Taylor-linearized
standard errors that agree with \stcmd{svy: poisson} on both point
estimates and standard errors; see
section~\ref{sec:svy}. \stcmd{svy bootstrap:} and
\stcmd{svy jackknife:} are also supported on all paths; \stcmd{absorb()}
and \stcmd{fast} are restricted to these resampling-based variants,
since Taylor linearization requires per-observation scores that are not
available after absorption or cell acceleration.
...
if \stcmd{stpois} does not converge, the recommended diagnostic steps
are:
...
The typical episode-split workflow starts with one record per subject,
splits spells into shorter episodes, adds indicators for the resulting
time bands to model the baseline hazard, and then estimates the
piecewise-constant hazard model. The following self-contained example
illustrates each step.

We simulate 1,000 subjects observed for up to ten years, with age, sex,
and log-income as covariates:
...
\noindent The dataset at this point has one record per subject, with
\stcmd{\_t0 = 0}, \stcmd{\_t = years}, and \stcmd{\_d = event}.
\stcmd{stsplit} splits each spell into annual episodes:
...
\noindent After splitting, each original subject-year becomes a separate
episode record, with \stcmd{\_t0} and \stcmd{\_t} marking the episode
boundaries. The variable \stcmd{fu} records the follow-up band (0 =
first year, 1 = second year, and so on). The failure indicator
\stcmd{\_d} is 1 only in the episode in which the event occurred and 0
in all preceding episodes. The exposure for each record,
\(\stcmd{\_t} - \stcmd{\_t0}\), is one year except possibly for the last
episode, which may be shorter.

A piecewise-constant hazard model with ten follow-up bands, a linear age
effect, and sex is then estimated on the episode file:
...
\noindent The first command uses the standard path and produces
identical results to \stcmd{streg, distribution(exponential)} on the
same data. The second uses the fast path, which treats \stcmd{i.fu} as
the cell-defining categorical term and estimates \stcmd{age},
\stcmd{female}, and \stcmd{income} at the individual level from
exponentially tilted within-cell moments. The third absorbs the ten
follow-up dummies, estimating only the covariate coefficients; because
the follow-up baseline hazard is of secondary interest here, absorption
avoids placing nine dummy columns in the output while still controlling
for duration dependence. The fast and absorb approaches can be combined:
\stcmd{stpois age female income i.edu, fast absorb(fu region) vce(cluster region)}
absorbs two high-dimensional nuisance effects simultaneously and
clusters the standard errors, while the individual-level covariates and
the education cells are estimated from the full episode file.

\subsection{Survey data}\label{survey-data}

\label{sec:svy}

\stcmd{stpois} declares \stcmd{properties(st svyr svyb svyj)}, so
\stcmd{svyset} data can be analyzed with the full \stcmd{svy:} prefix
family. For Taylor-linearized standard errors, \stcmd{svy: stpois} calls
\stcmd{stpois} to obtain point estimates, then requests per-observation
scores \(d_i - \hat{\mu}_i\) from \stcmd{stpois\_p} and constructs the
sandwich variance matrix using the survey design; this is the same
linearization as \stcmd{svy: poisson} and produces identical results:
...
\noindent \stcmd{stpois} inherits sampling weights through the
\stcmd{stset} weight mechanism: when \stcmd{svy:} is active, it injects
the survey \stcmd{pweight} into the dataset's \stcmd{stset}
characteristics before calling the subcommand, and \stcmd{stpois} reads
those characteristics to apply the weights to the internal
\stcmd{poisson} call. This mirrors the behavior of \stcmd{streg} under
\stcmd{svy:} and means that \stcmd{svy: stpois} and \stcmd{svy: poisson}
agree on both point estimates and linearized standard errors to at least
\(10^{-6}\). \stcmd{svy, subpop():} subpopulation estimation and
stratified or multi-stage designs are handled automatically by Stata's
\stcmd{svy} machinery.

Taylor linearization requires per-observation scores and is therefore
restricted to the standard path. With \stcmd{absorb()} or \stcmd{fast},
use \stcmd{svy bootstrap: stpois} or \stcmd{svy jackknife: stpois}:
these resampling-based variance estimators re-run the full estimation on
each replicate and are therefore valid for any estimation path.
...
\item \textbf{svy:} on the standard path, \stcmd{svy: stpois} agrees with \stcmd{svy: poisson} on point estimates and Taylor-linearized standard errors to $10^{-6}$ (observed agreement at the level of floating-point rounding, $\sim 10^{-15}$), including with stratified designs and subpopulation estimation.
\end{enumerate}
...
\stcmd{stpois} packages four things that belong together but have
...
effects without external dependencies, cell-accelerated estimation that
retains continuous covariates, and design-based variance estimation via
the \stcmd{svy:} prefix. The standard and fast paths are algebraically
exact---they solve the same likelihood as \stcmd{poisson} with no
mathematical approximation---and the \stcmd{absorb()} path converges to
the same maximum-likelihood optimum to within its specified tolerance;
the test suite holds coefficients \emph{and} variance estimators, not
merely point estimates, to the \stcmd{streg} and \stcmd{poisson}
benchmarks at \(10^{-6}\) (typically \(\sim 10^{-8}\)), and
factor-variable interactions are supported throughout, including
interacted fixed effects. Weights are accepted on every path, the fast
and absorbed paths can be combined, predictions after \stcmd{absorb()}
include the stored fixed-effect contributions, and \stcmd{svy: stpois}
produces Taylor-linearized standard errors that agree with
\stcmd{svy: poisson} to at least \(10^{-6}\). The command, its help
file, the test suite, and the benchmark scripts are available from the
author's repository and are installable with \stcmd{net install}.
