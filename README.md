...
- `fast(moments)` is exact (coefficients and SEs match full `poisson` to numerical precision). `fast(offset)` SEs are conditional on first-stage estimates by default; `mtopel` propagates first-stage uncertainty exactly.
